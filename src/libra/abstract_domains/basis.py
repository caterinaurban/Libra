...
from libra.abstract_domains.lattice import Lattice, ArithmeticMixin
from libra.abstract_domains.state import State
from libra.abstract_domains.store import Store
from libra.core.expressions import VariableIdentifier, Expression, \
    BinaryBooleanOperation, ExpressionVisitor, Literal, \
    Input, UnaryArithmeticOperation, BinaryArithmeticOperation, \
...
class Basis(Store, State, metaclass=ABCMeta):
    ...
    class ExpressionEvaluation(ExpressionVisitor):
        ...
        def visit_Literal(self, expr: Literal, state=None, evaluation=None):
            ...
            evaluation[expr] = state.lattice(**state.arguments).top()
            return evaluation
        ...
        def visit_UnaryArithmeticOperation(self, expr, state=None, evaluation=None):
            ...
            if expr.operator == UnaryArithmeticOperation.Operator.Add:
                ...
                if isinstance(value, ArithmeticMixin):
                    ...
                    evaluated[expr] = state.lattice(**state.arguments).top()
                return evaluated
