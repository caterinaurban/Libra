...
def main():
    ...
    if args.domain == 'boxes':
        BiasAnalysis(args.specification, domain=AbstractDomain.BOXES, difference=args.lower, widening=args.upper).main(args.neural_network)
    if args.domain == 'symbolic':
        BiasAnalysis(args.specification, domain=AbstractDomain.SYMBOLIC2, difference=args.lower, widening=args.upper).main(args.neural_network)
    if args.domain == 'deeppoly':
        BiasAnalysis(args.specification, domain=AbstractDomain.DEEPPOLY, difference=args.lower, widening=args.upper).main(args.neural_network)
